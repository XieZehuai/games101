...
public:
    ...
    Triangle(Vector3f _v0, Vector3f _v1, Vector3f _v2, Material* _m = nullptr) : v0(_v0), v1(_v1), v2(_v2), material(_m)
    {
        e1     = v1 - v0;
        e2     = v2 - v0;
    ...
    void         getSurfaceProperties(const Vector3f& P,
                                      const Vector3f& I,
    ...
    MeshTriangle(const std::string& filename)
    {
        ...
        for (int i = 0; i < mesh.Vertices.size(); i += 3)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                ...
                min_vert =
                    Vector3f(std::min(min_vert.x, vert.x), std::min(min_vert.y, vert.y), std::min(min_vert.z, vert.z));
                max_vert =
                    Vector3f(std::max(max_vert.x, vert.x), std::max(max_vert.y, vert.y), std::max(max_vert.z, vert.z));
            }
...
inline Vector3f Triangle::evalDiffuseColor(const Vector2f&) const { return Vector3f(0.5, 0.5, 0.5); }
